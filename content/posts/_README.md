...
- **HTML 주석 `<!-- ... -->`은 그대로 두셔도 됩니다.** MDX 파서는 `<!`에서 멈추지만,
  `lib/posts.ts`의 `stripHtmlComments`가 렌더 전에 제거하므로 화면에도 안 나오고 빌드도 안 깨집니다.
  `{/* ... */}`(MDX 주석)도 물론 됩니다. 코드펜스 안의 `<!--`나 mermaid의 `-->`는 건드리지 않습니다.
- `<`, `{` 를 본문에 그대로 쓸 때는 `` `코드` `` 로 감싸거나 `\{`로 이스케이프합니다.
