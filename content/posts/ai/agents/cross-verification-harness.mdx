---
title: "작성자 ≠ 검증자 — Claude Code × Codex 교차검증 하네스 구축기"
date: "2026-07-09"
...
> 🔁 **같은 모델이 만들고 스스로 검증하면, 같은 맹점을 두 번 통과시킨다.** AI 에이전트에게 구현을 맡기는 시대에 "누가 그 결과를 믿어주는가"라는 신뢰 문제를, **작성자(Claude Code) ≠ 검증자(Codex)** 역할 분리로 풀어낸 하네스 구축 기록입니다. 설계 → 구현 → 도그푸딩 → 그리고 **하네스가 첫날 자기 자신의 버그 12건을 잡아낸** 이야기까지.
...
### 3-3. 기계 게이트 — `scripts/agent-verify.mjs`
...
### 3-4. 검증자 자동 호출 — `scripts/agent-codex-review.mjs`
...
### 3-5. Handoff — 검증자에게 넘기는 유일한 컨텍스트
