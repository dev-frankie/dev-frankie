...
> **문구 한 줄 바꾸는 데 온 팀이 움직였습니다.** 개발자는 JSON 세 개를 손보고, 번역자는 시트를 오갔고, 그 사이 번역은 자꾸 어긋났습니다. 이 글은 그 다국어(i18n) 플로우를 **컬럼 단위 소유권** 원칙 하나로 다시 설계해, *개발자는 `t("...")`만, 번역자는 ko/ja만* 만지면 되도록 만든 기록입니다. (next-intl 기반 · en/ko/ja 3개 로케일)

이 글을 쓰게 된 이유는 단순합니다. 다국어 하나를 늘리는 일이 유독 손이 많이 갔고, 그때마다 "왜 이렇게까지 번거로운가"를 계속 곱씹었기 때문입니다. 화려한 기술이 담긴 이야기는 아닙니다. **충돌을 어디서 없앨지, 누가 무엇의 주인일지** 같은 가장 기본적인 질문을 하나씩 풀어간 과정에 가깝습니다.
...
> 🌏 **스프레드시트 `i18n_translations` 탭에서 ko/ja를 채우고, "번역 완료" 버튼을 누르면 끝입니다.**
...
> 📷 (스크린샷: fine-grained PAT에 Contents read/write 권한을 부여하는 화면)

> 📷 (스크린샷: Apps Script에 `notifyTranslationsReady()`를 작성한 화면)
...
/github subscribe datumo-platform/datumo-platform-front pulls +label:"i18n"
```
...
gcloud auth login && gcloud config set project datumo-platform
...
> ⚠️ **원시 `build-en --prune`을 함부로 쓰지 마세요.** 동적 호출의 키(예: `SELF_EVALUATION`, `SourceTypeEdited`)는 정적 추출이 못 봅니다. "추출기가 못 찾음 ≠ 죽은 키"입니다. 전부 삭제하면 살아 있는 동적 키가 날아가 화면이 깨집니다.
...
"문구 한 줄 바꾸는 데 온 팀이 동원되던" 상태에서, **개발자는 `t()`, 번역자는 ko/ja, 그 사이는 CI**로 역할을 잘라낸 것이 이번 작업의 핵심입니다. 충돌을 락으로 막는 대신 *한 셀에 주인을 하나만* 두어 구조적으로 제거했고, 사람 손을 거치던 나머지는 스크립트와 버튼으로 넘겼습니다. 대단한 기술은 없었지만, **누가 무엇의 주인인지**를 한 번 분명히 그어 두니 나머지가 자연스럽게 따라왔습니다. 남은 일은 운영하며 규칙(orphan 정리·동적 네임스페이스 분리)을 다듬는 것뿐입니다.

- 개발자: `t()` 쓰고 커밋 → 끝
- 번역자: ko/ja 채우고 버튼 → 끝
- 충돌: 한 셀에 주인이 하나뿐 → 구조적으로 0
