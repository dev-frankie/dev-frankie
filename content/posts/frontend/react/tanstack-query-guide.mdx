---
title: "TanStack Query 상편 — 서버 상태를 라이브러리에 맡긴다는 것"
date: "2026-07-11"
description: "useQuery를 쓰는 법이 아니라, 왜 이 옵션들이 이렇게 생겼는지. staleTime과 gcTime이 계속 헷갈리는 진짜 이유와, 이 라이브러리가 안 맞는 경우까지."
...
> - **1편 (이 글)** — 서버 상태를 라이브러리에 맡긴다는 것: 사용법과 옵션, 그리고 그 설계 판단
> - [2편 — 소스로 읽는 내부 동작](./tanstack-query-deep-dive): `useBaseQuery`부터 `notifyManager`까지 v5.95.0 소스 순회

`useState`와 `useEffect`로 API를 부르다 보면 어느 순간 같은 코드를 반복해서 쓰고 있습니다. 로딩 플래그, 에러 상태, 언마운트 가드, 중복 요청 방지, 포커스 복귀 시 갱신. 이걸 훅 하나로 묶어내면 그게 사실상 캐시 라이브러리의 축소판입니다.

TanStack Query는 그 반복을 대신 해주는 도구입니다. 그런데 막상 쓰기 시작하면 옵션 이름에서 막힙니다. `staleTime`과 `gcTime`이 뭐가 다른지, `status`와 `fetchStatus`는 왜 둘인지, `invalidate`는 왜 refetch가 아닌지.
